...
// ===== COMPLETE PVP PATCH =====
let enemyMoney=100000;
let enemyDiamonds=1000;
let matchTime=300;

function syncEnemyHud(){
 let a=document.getElementById('enemyMoney');
 let b=document.getElementById('enemyDiamonds');
 let c=document.getElementById('jokerCount');
 if(a)a.textContent=formatNumber(enemyMoney);
 if(b)b.textContent=formatNumber(enemyDiamonds);
 if(c)c.textContent=jokers;
}

setInterval(()=>{
 if(matchTime>0){
   matchTime--;
   let e=document.getElementById('matchTimer');
   if(e){
      let m=String(Math.floor(matchTime/60)).padStart(2,'0');
      let s=String(matchTime%60).padStart(2,'0');
      e.textContent='⏳ Tempo Restante: '+m+':'+s;
   }
 }
},1000);

const _oldOpenBox=openBox;
openBox=function(type){
 if(Math.random()<0.01){ findJoker(); syncEnemyHud(); return; }

 const cfg={
  boxSmall:{s:0.50,win:0.02,lose:0.02},
  boxMedium:{s:0.70,win:0.05,lose:0.05},
  boxLarge:{s:0.80,win:0.10,lose:0.10},
  boxHuge:{s:0.90,win:0.20,lose:0.20},
  boxRoyal:{s:0.90,win:0.30,lose:0.10},
  boxCosmic:{s:0.90,win:0.50,lose:0.15},
  boxBaphomet:{s:0.90,win:0.80,lose:0.25}
 };

 if(!cfg[type]) return _oldOpenBox(type);

 const c=cfg[type];

 if(Math.random()<c.s){
   let v=Math.floor(enemyMoney*c.win);
   enemyMoney-=v;
   money+=v;
   showPopup('⚔️ Roubou '+formatNumber(v));
 }else{
   let v=Math.floor(money*c.lose);
   money-=v;
   showPopup('💥 Falhou e perdeu '+formatNumber(v));
 }
...
try{
 localStorage.removeItem("moonwalkSave");
}catch(e){}
...
updateUI = function(){
 ...
 const mic=document.getElementById("michael");
 if(totalMoney < 50){
   if(stage) stage.textContent="Michael Mendigo";
